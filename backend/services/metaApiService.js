...
class MetaApiService {
  ...
  async connect() {
    ...
    try {
      ...
      // Fire-and-forget: subscribing 116 symbols serially can hang for minutes
      // (subscribeToMarketData awaits broker response, some symbols missing from LP).
      // Prices already stream via terminalState/PriceListener as soon as sync completes,
      // so don't block connect() on this.
      this.subscribeToSymbols(SUPPORTED_SYMBOLS).catch(e =>
        console.error('[MetaApi] Background subscribe error:', e?.message)
      )
  ...
  getPrice(symbol) {
    ...
    // No live data — fall back to static price (mirrors the Infoway behavior).
    // Required so symbols the LP doesn't supply still appear in the instrument list
    // instead of vanishing from the UI.
    const fb = FALLBACK_PRICES[symbol]
    return fb ? { ...fb, time: Date.now(), fallback: true } : null
  }

  // Snapshot of every supported symbol — live where available, fallback otherwise.
  // Consumed by server.js streamPrices to populate priceCache and broadcast to UI.
  getAllPrices() {
    const prices = {}
    for (const symbol of SUPPORTED_SYMBOLS) {
      const p = this.getPrice(symbol)
      if (p && p.bid && p.ask) prices[symbol] = p
    }
    return prices
  }
